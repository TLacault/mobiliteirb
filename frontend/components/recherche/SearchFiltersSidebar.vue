...
const departureAutocomplete = usePlaceAutocomplete();
const arrivalAutocomplete = usePlaceAutocomplete();
const departureSuggestions = departureAutocomplete.suggestions;
const departureSuggestionsNotEmpty =
  departureAutocomplete.suggestionsNotEmpty;
const arrivalSuggestions = arrivalAutocomplete.suggestions;
const arrivalSuggestionsNotEmpty = arrivalAutocomplete.suggestionsNotEmpty;

function selectDepartureSuggestion(suggestion) {
  departure.value = formatPlaceSuggestion(suggestion);
  departureAutocomplete.close();
}

function selectArrivalSuggestion(suggestion) {
  arrival.value = formatPlaceSuggestion(suggestion);
  arrivalAutocomplete.close();
}
          ...
          <div class="places-fields">
            <label class="field-wrapper">
              <input
                ...
                type="text"
                class="filter-input"
                @input="(e) => departureAutocomplete.handleInput(e.target.value)"
                @focus="departureAutocomplete.handleFocus"
                @blur="departureAutocomplete.handleBlur"
              />

              <ul
                v-if="departureSuggestionsNotEmpty && departureSuggestions.length"
                class="autocomplete-dropdown"
              >
                <li
                  v-for="s in departureSuggestions"
                  :key="`${s.cityName}-${s.countryName}`"
                  class="autocomplete-item"
                  @mousedown.prevent="selectDepartureSuggestion(s)"
                >
            ...
            <label class="field-wrapper">
              <input
                ...
                type="text"
                class="filter-input"
                @input="(e) => arrivalAutocomplete.handleInput(e.target.value)"
                @focus="arrivalAutocomplete.handleFocus"
                @blur="arrivalAutocomplete.handleBlur"
              />

              <ul
                v-if="arrivalSuggestionsNotEmpty && arrivalSuggestions.length"
                class="autocomplete-dropdown"
              >
                <li
                  v-for="s in arrivalSuggestions"
                  :key="`${s.cityName}-${s.countryName}`"
                  class="autocomplete-item"
                  @mousedown.prevent="selectArrivalSuggestion(s)"
                >
                  {{ s.cityName }}, {{ s.countryName }}
                </li>
              </ul>
