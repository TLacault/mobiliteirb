...
async function getStepEstimation(data) {
  ...
  if (["plane"].includes(transportMode)) {
    try {
      const startCoords =
        typeof origin === "string" ? await getCoordinates(origin) : origin;
      const endCoords =
      ...
      throw new Error(
        `Impossible de géolocaliser les adresses "${origin}" ou "${destination}". Vérifiez l'orthographe.`,
      );
    }
  } else {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Erreur API Google Maps (${response.status}). Vérifiez votre quota ou votre clé API.`,
        );
      }
      if (!result.routes || result.routes.length === 0) {
        console.log("No route found:", result);
        throw new Error(
          `Aucun itinéraire trouvé ${googleTravelMode} entre "${origin}" et "${destination}". Essayez un autre mode de transport ou vérifiez les adresses.`,
        );
      }

      distanceKm = result.routes[0].distanceMeters / 1000;
      if (
        (["tram", "metro", "bus_gasoline", "bus_electric"].includes(
          transportMode,
        ) &&
          distanceKm > 100) ||
        (["train_paris"].includes(transportMode) && distanceKm > 150)
      ) {
        throw new Error(
          `Le mode "${transportMode}" n'est pas réaliste pour une distance de ${Math.round(
            distanceKm,
          )}km.`,
        );
      }
      durationMin = parseInt(result.routes[0].duration) / 60;
      if (transportMode === "bus_gasoline_long_haul") durationMin *= 1.2;
    } catch (error) {
      console.error("ComputeRoutes Error:", error.message);
      if (
        error.message.includes("Aucun itinéraire") ||
        error.message.includes("Erreur API")
      ) {
        throw error;
      }
      throw new Error(
        `Erreur lors du calcul de l'itinéraire : ${error.message}`,
      );
    }
  }
  ...
  try {
    ...
    throw new Error(
      `Impossible de calculer les émissions CO₂ pour le mode "${transportMode}". Vérifiez que ce mode est supporté.`,
    );
  }
...
module.exports = { getStepEstimation, getCoordinates };
