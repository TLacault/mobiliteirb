...
const { generateMobilityCsv } = require("../utils/exportCsv");
const { generateMobilityPdfStyled } = require("../utils/exportPdfStyled");
...
async function exportMobility(req, res) {
  try {
    ...
    if (mode === "csv") {
      const csvContent = generateMobilityCsv(mobility);
    ...
    if (mode === "rapport") {
      try {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="Synthese_Mobilite_${id}.pdf"`,
        );
        const pdfBuffer = await generateMobilityPdfStyled(mobility);
