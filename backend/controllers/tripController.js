...
/**
 * GET /api/v1/trips/:id_mobilite
 * Récupérer la liste des uuid des trajets d'une mobilité
 */
async function getAllTrips(req, res) {
  try {
    const mobiliteId = req.params.mobiliteId;
...
async function getTripStats(req, res) {
    ...
    if (!tripId) {
      ...
      where: { id: tripId },
        ...
        const totalDistance = trip.steps.reduce((sum, s) => sum + (s.distance ?? 0), 0);
